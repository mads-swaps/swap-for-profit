...
def prepare_features(start, end, x_data, info_dict):
    ...
    info_dict['data_subset']['momentum'] = info_dict['data_subset']['returns'].rolling(5).mean().shift(1)
    info_dict['data_subset']['volatility'] = info_dict['data_subset']['returns'].rolling(20).std().shift(1)
    info_dict['data_subset']['distance'] = (info_dict['data_subset']['close'] -
                                            info_dict['data_subset']['close'].rolling(50).mean()).shift(1)

    info_dict['data_subset'].dropna(inplace=True)
    info_dict['feature_columns'].extend(['momentum', 'volatility', 'distance'])
...
def plot_results(info_dict):
    ...
    title = f'{info_dict["portfolio"]["starting_coin"]}-{info_dict["portfolio"]["pair_coin"]} additional features | \
