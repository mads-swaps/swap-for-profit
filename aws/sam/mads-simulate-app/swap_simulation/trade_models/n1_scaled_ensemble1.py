...
def ensemble_predict(x_data, info_dict):
    numlkb_dict = {'A': 15, 
                   'B': 3,
                   'C': 0}
    
    models = pickle.load(open(info_dict['model_filepath'], 'rb'))
    threshold = info_dict['model_threshold']
    total_precision = 0
    scores = np.zeros(len(x_data))

    for m in models:
        columns = get_columns(x_data, numlkb_dict[m['lkb']])
        total_precision += m['pr']
        scores += m['clf'].predict(x_data[columns])  * m['pr']
...
def make_decision(x_data, extra_data, info_dict):
    ...
    pred = ensemble_predict(x_data, info_dict)
