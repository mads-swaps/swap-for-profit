...
def load_model_with_config(config, training_set=None, force_train=False):
    ...
    path = config.get('model_path', "./models")
    ...
    if os.path.exists(f) and not force_train:
        ...
        net = config['model'](inputSize=training_set.num_X, outputSize=training_set.num_y, lookback_count=24, model_definition = config['model_definition'])
        loss_func = torch.nn.MSELoss()  # this is for regression mean squared loss
#         loss_func = torch.nn.BCEWithLogitsLoss()  # this is for regression mean squared loss
#        loss_func = torch.nn.L1Loss()
        optimizer = torch.optim.AdamW(net.parameters(), lr=config['lr'], weight_decay=config['weight_decay'], eps=1e-08, amsgrad=False)
...
def save_model_with_config(config, **kwargs):
    path = config.get('model_path', f"./models")
...
def create_model(in_size, out_size, lookback_size, **kwargs):
    model = CNNModel(inputSize=in_size,
                      outputSize=out_size,
                     lookback_count = lookback_size,
                      **kwargs)
    return model
